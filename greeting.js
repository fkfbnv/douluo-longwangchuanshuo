...
.ring-item .dot.white{
  ...
  box-shadow:0 0 6px rgba(200,200,200,0.15);
}
.ring-item .dot.white::after{
  content:'';position:absolute;top:2px;left:2px;right:2px;bottom:2px;
  border-radius:50%;border:1px dashed rgba(160,160,160,0.3);
}
.ring-item .dot.yellow{
  background:#fcd34d;
  border:1px solid #d4a017;
  box-shadow:0 0 12px rgba(252,211,77,0.3);
}
.ring-item .dot.yellow::after{
  content:'';position:absolute;top:0;left:0;right:0;bottom:0;
  border-radius:50%;background:radial-gradient(circle at 30% 30%, rgba(255,255,200,0.4), transparent 60%);
}
.ring-item .dot.purple{
  background:#9b6bcc;
  border:1px solid #7a4a9a;
  box-shadow:0 0 14px rgba(155,107,204,0.25);
}
.ring-item .dot.purple::after{
  content:'';position:absolute;top:1px;left:1px;right:1px;bottom:1px;
  border-radius:50%;background:radial-gradient(circle at 40% 30%, rgba(200,180,255,0.3), transparent 60%);
}
.ring-item .dot.black{
  background:#2a2a2a;
...
.ring-item .dot.black::after{
  content:'';position:absolute;top:2px;left:2px;right:2px;bottom:2px;
  border-radius:50%;border:1px solid rgba(80,70,60,0.3);
  background:radial-gradient(circle at 30% 30%, rgba(100,100,100,0.2), transparent 60%);
}
.ring-item .dot.red{
  background:#dc3545;
  border:2px solid #ffd700;
  box-shadow:0 0 20px rgba(220,53,69,0.4),inset 0 0 10px rgba(255,215,0,0.15);
}
.ring-item .dot.red::after{
  content:'';position:absolute;top:-1px;left:-1px;right:-1px;bottom:-1px;
  border-radius:50%;border:1px solid rgba(255,215,0,0.3);
}
.ring-item .dot.orangegold{
  background:#e8a317;
  border:2px solid #ff8c00;
  box-shadow:0 0 25px rgba(232,163,23,0.45);
}
.ring-item .dot.orangegold::after{
  content:'';position:absolute;top:1px;left:1px;right:1px;bottom:1px;
  border-radius:50%;
...
.ring-item .dot.rainbow{
  background:conic-gradient(red,#ff6b00,yellow,#00ff00,#0066ff,#8b00ff,red);
  border:2px solid #ffd700;
  box-shadow:0 0 35px rgba(255,215,0,0.5),0 0 60px rgba(255,215,0,0.15);
}
.ring-item .dot.rainbow::after{
  content:'';position:absolute;top:0;left:0;right:0;bottom:0;
  border-radius:50%;background:radial-gradient(circle at 30% 30%, rgba(255,255,255,0.3), transparent 50%);
}
...
/* === 魂灵年限颜色（同步增强） === */
.spirit-dot{
  width:11px;height:11px;border-radius:50%;flex-shrink:0;display:inline-block;vertical-align:middle;margin-right:2px;
}
...
.export-area .footer .btn-export-persona{
  font-size:.8em;
  padding:1px 8px;
  border-radius:10px;
  border:1px solid rgba(255,215,0,0.06);
...
`;
    ...
    (function() {
        ...
        var fields = {
            ...
            soulTitle: $('soulTitle'),
            titleName: $('titleName'),
            titleNameWrap: $('titleNameWrap'),
            sp: $('sp'),
            spiritDesc: $('spiritDesc'),
            spiritWarn: $('spiritWarn'),
            ban: $('ban'),
            bal: $('bal'),
            bat: $('bat'),
            armorGrade: $('armorGrade'),
            mechaLevel: $('mechaLevel'),
            mechaCustom: $('mechaCustom'),
            mechaCustomWrap: $('mechaCustomWrap'),
            secondProf: $('secondProf'),
            secondLevel: $('secondLevel'),
            secondProfDesc: $('secondProfDesc'),
            st: $('st'),
            stDesc: $('stDesc'),
            customSpecialName: $('customSpecialName'),
            customSpecialDesc: $('customSpecialDesc'),
            customSpecialWrap: $('customSpecialWrap'),
            ...
            stageDisplayId: $('stageDisplayId'),
            stageDisplayName: $('stageDisplayName'),
            stageDisplayAge: $('stageDisplayAge'),
            stageDisplayDate: $('stageDisplayDate'),
            stageEventSelect: $('stageEventSelect'),
            stageEventDisplay: $('stageEventDisplay'),
            spiritDot: $('spiritDot')
        };
        ...
        var fateModal = $('fateModal'),
            fateBody = $('fateBody');
        var soulComment = $('soulComment');
        var toast = $('toast');
        var backpackGrid = $('backpackGrid'),
            backpackInput = $('backpackInput');
        var armorPartsGroup = $('armorPartsGroup'),
            bonePartsGroup = $('bonePartsGroup');
        var titleEpithet = $('titleEpithet');
        var soulDeviceGrid = $('soulDeviceGrid'),
            soulDeviceInput = $('soulDeviceInput'),
            soulDeviceQuality = $('soulDeviceQuality');
        ...
        var campIdentityLinks = {
            '无': ['无'],
            '史莱克学院': ['外院学员', '内院学员', '海神阁弟子', '史莱克七怪', '工读生', '监察团成员'],
        ...
        // 通用身份列表（用于阵营身份下拉框的完整列表）
        var allIdentityOptions = [
            '无', '外院学员', '内院学员', '海神阁弟子', '史莱克七怪', '工读生', '监察团成员',
            '外门弟子', '内门弟子', '斗魂堂成员', '敏堂成员', '力堂成员', '御堂成员', '供奉堂成员',
            '灵塔守卫', '魂灵师', '传灵使', '长老', '副塔主', '塔主亲传',
            '预备战神', '战神', '神将',
            '普通士兵', '精英战士', '血神卫', '血神营成员',
            '皇室成员', '贵族', '御林军', '边防军', '供奉殿成员',
            '本体战士', '核心弟子', '宗主亲传',
            '家族精英', '龙卫', '家族长老', '泰坦卫',
            '自由魂师', '散修',
            '普通平民', '魂师世家', '宗门嫡系', '皇室宗亲', '隐世家族', '孤儿', '魂兽化形',
            '神祇后裔', '龙族后裔'
        ];
        ...
        // ★★★ 精神力-魂环年限映射 ★★★
        function getMaxRingYearBySpirit(spiritLevel) {
            var map = {
                '灵元境': '百年',
                '灵通境': '千年',
                '灵海境': '万年',
            ...
            return map[spiritLevel] || '百年';
        }

        function getRingYearWeight(year) {
            var weights = {
                '十年': 1,
                '百年': 2,
                '千年': 3,
        ...
        function getRingCount(level) {
            var lv = Number(level);
            if (isNaN(lv) || lv < 0) return 0;
            if (lv <= 9) return 0;
            if (lv <= 19) return 1;
            if (lv <= 29) return 2;
            if (lv <= 39) return 3;
        ...
        function getSoulComment(level) {
            ...
            if (lv <= 9) return '🌱 初出茅庐';
            if (lv <= 19) return '🌿 魂师之境';
            if (lv <= 29) return '💪 大魂师！';
            if (lv <= 39) return '⚔️ 魂尊！';
            if (lv <= 49) return '🔥 魂宗！';
            if (lv <= 59) return '🌟 魂王！';
            if (lv <= 69) return '👑 魂帝！';
            if (lv <= 79) return '✨ 魂圣！';
        ...
        function getArmorGrade(level) {
            var map = {
                '无': { text: '无斗铠', color: '#64748b' },
                '一字斗铠': { text: '一字·白', color: '#b5b5b5' },
                '二字斗铠': { text: '二字·黄', color: '#c0a868' },
                '三字斗铠': { text: '三字·紫', color: '#9b6bcc' },
                '四字斗铠': { text: '四字·黑', color: '#4a3a2a' },
                '五字斗铠（神级）': { text: '五字·红', color: '#b06050' },
                '六字斗铠（神级）': { text: '六字·金', color: '#d4b878' }
            };
            return map[level] || { text: level, color: '#64748b' };
        }
        ...
        function getSpiritClass(year) {
            ...
            return map[year] || 's-white';
        }

        function getSpiritColor(year) {
            var map = {
                '十年': '#e8e8e8',
                '百年': '#fcd34d',
                '千年': '#9b6bcc',
        ...
        function validateRings() {
            ...
            rings.forEach(function(ring, idx) {
                ...
                if (weight > maxWeight) {
                    hasIssue = true;
                    issues.push('第' + (idx + 1) + '环（' + ring.year + '）超出精神力上限（' + maxYear + '）');
                }
            });
        ...
        // ===== 神器锁定 =====
        function updateWeaponLock() {
            var btn = $('btnLockWeapon');
            var hint = $('weaponLockHint');
            if (weaponLocked) {
                btn.textContent = '🔒 已锁定';
                btn.style.borderColor = 'rgba(74,201,176,0.3)';
                btn.style.color = '#4ac9b0';
                hint.textContent = '🔒 已锁定，切换特殊能力不会自动修改神器';
            } else {
                btn.textContent = '🔓 自动';
                btn.style.borderColor = 'rgba(255,215,0,0.1)';
                btn.style.color = '#94a3b8';
                hint.textContent = '🔓 自动模式，切换特殊能力会自动更新神器';
            }
        }
        ...
        function showPane(n) {
            ...
            if (n === 2) { updateBackpackByCamp();
                updateSoulDevicesByCamp();
        ...
        function syncTagsFromInput(type) {
            var input = type === 'personality' ? fields.personality : fields.traits;
            var container = type === 'personality' ? $('personalityTags') : $('traitsTags');
        ...
        function randomOrigin() {
            var idx = Math.floor(Math.random() * originList.length);
            var origin = originList[idx];
            fields.og_select.value = origin;
            fields.og.value = origin;
            updateCamp();
            updateBackpackByCamp();
            updateSoulDevicesByCamp();
            saveDraft();
            updateProgress();
            showToast('🎲 随机出身地：' + origin);
        }
        ...
        function updateIdentityByCamp() {
            ...
            allIdentityOptions.forEach(function(id) {
                if (!existing[id]) {
                    var opt = document.createElement('option');
                    opt.value = id;
                    opt.textContent = id;
        ...
        function filterBackpackByIdentity() {
            ...
            if (identity === '孤儿') {
                ...
                renderBackpack();
                saveDraft();
                showToast('身份为「孤儿」，已移除家族/贵族相关道具');
            }
            if (identity === '魂兽化形') {
                backpackItems = backpackItems.filter(function(item) {
                    return item.indexOf('户籍') === -1 &&
                        item.indexOf('居民证') === -1 &&
                        item.indexOf('通行证') === -1;
                });
                localStorage.setItem('backpack_items_v14', JSON.stringify(backpackItems));
        ...
        function updateSpecialDesc() {
            ...
            if (found && found.name === '自定义') {
                ...
                var sideText = found.side ? ' ⚠️ 副作用：' + found.side : '';
                fields.stDesc.textContent = '📖 ' + found.desc + sideText;
            } else {
        ...
        // ===== 魂环 =====
        function initRings() {
            var saved = localStorage.getItem('soul_rings_v14');
        ...
        function updateRingInfo() {
            ...
            if (info) {
                if (lv === '' || lv === undefined) {
                    ...
                    info.innerHTML = '<span class="match">✅ ' + count + '/' + expected + '</span>';
                } else if (count < expected) {
                    info.innerHTML = '<span class="mismatch">⚠️ ' + count + '/' + expected + '</span>';
                } else {
                    info.innerHTML = '<span class="mismatch">⚠️ 超限</span>';
                }
            }
            validateRings();
        }

        function syncRingsToLevel() {
            var lv = getVal('csr');
            var expected = getRingCount(lv);
            if (expected === 0 && rings.length > 0 && rings[0].year) {
                rings = [];
            } else if (expected > rings.length) {
                for (var i = rings.length; i < expected; i++) {
                    rings.push({ year: '百年' });
                }
            } else if (expected < rings.length && expected > 0) {
                rings = rings.slice(0, expected);
            }
            localStorage.setItem('soul_rings_v14', JSON.stringify(rings));
            renderRings();
            updateRingInfo();
        ...
        function renderArmorParts() {
            ...
            $('armorDoneCount').textContent = done;
            $('armorTotalCount').textContent = armorPartNames.length;
            var pct = Math.round((done / armorPartNames.length) * 100);
            $('armorPercentDisplay').textContent = pct + '%';
        }

        function updateArmorStats() {
            var done = 0;
            armorPartNames.forEach(function(name) { if (armorParts[name]) done++; });
            $('armorDoneCount').textContent = done;
            var pct = Math.round((done / armorPartNames.length) * 100);
            $('armorPercentDisplay').textContent = pct + '%';
        }
        ...
        function updateBoneStats() {
            var done = 0;
            bonePartNames.forEach(function(name) { if (boneParts[name]) done++; });
            $('boneDoneCount').textContent = done;
        }
        ...
        function updateBackpackByCamp() {
            ...
            if (identity === '孤儿') {
                allPresets = allPresets.filter(function(item) {
                    return item.indexOf('徽章') === -1 &&
                        item.indexOf('家族') === -1 &&
                        item.indexOf('贵族') === -1 &&
                        item.indexOf('皇室') === -1 &&
        ...
        // ===== 其他联动 =====
        function updateMechaCustom() { if (getSel('mechaLevel') === '红级（自定义）') { fields.mechaCustomWrap.style.display = 'block'; } else { fields.mechaCustomWrap.style.display = 'none';
                fields.mechaCustom.value = ''; } }
        ...
        function updateInnateOptions() {
            ...
            select.innerHTML = '';
            options.forEach(function(o) { var opt = document.createElement('option');
                opt.value = o.val;
        ...
        function updateCamp() {
            var origin = getSel('og_select');
            var map = { '东海城': '自由魂师', '傲来城': '自由魂师', '史莱克城': '史莱克学院', '天斗城': '天斗帝国', '星罗城': '星罗帝国', '灵波城': '传灵塔', '明都': '星罗帝国', '星斗大森林外围': '自由魂师', '极北之地': '自由魂师', '西海城': '自由魂师', '南海城': '自由魂师', '北海城': '自由魂师', '落日森林': '自由魂师', '龙谷外围': '自由魂师', '血神军团驻地': '血神军团', '魔鬼岛': '史莱克学院', '无尽山脉入口': '自由魂师', '海神岛': '史莱克学院' };
        ...
        function updateSoulComment() { var lv = getVal('csr'); var comment = getSoulComment(lv);
            soulComment.textContent = comment;
            soulComment.className = 'soul-comment' + (getVal('csr') !== '' ? ' active' : ''); }

        function updateArmorGrade() { var level = getSel('bal'); var info = getArmorGrade(level);
        ...
        function collectData() {
            ...
            var armorDone = 0;
            armorPartNames.forEach(function(n) { if (armorParts[n]) armorDone++; });
            var boneDone = 0;
            bonePartNames.forEach(function(n) { if (boneParts[n]) boneDone++; });
            ...
            return {
                ...
                "斗铠": { "斗铠名称": getVal('ban') || '', "斗铠等级": getSel('bal'), "斗铠侧重": getSel('bat'), "斗铠品级": getArmorGrade(getSel('bal')).text, "斗铠部件完成": armorDone + '/' + armorPartNames.length },
        ...
        function updateProgress() {
            ...
            for (var i = 0; i < fieldIds.length; i++) {
                var id = fieldIds[i];
                var el = fields[id];
                if (el && el.value && el.value.trim() !== '' && el.value !== '无' && el.value !== '无斗铠' && el.value !== '-- 请选择 --' && el.value !== '无名小卒') filled++;
            }
        ...
        function saveDraft() {
            try {
                ...
                data._weaponLocked = weaponLocked;
        ...
        function loadDraft() {
            try {
                ...
                if (data._rings && Array.isArray(data._rings) && data._rings.length) { rings = data._rings;
                    renderRings(); }
                ...
                if (data._specialTier) {
                    ...
                    if (data._specialValue) {
                        setTimeout(function() {
                            fields.st.value = data._specialValue;
                            updateSpecialDesc();
                            updateSpecialWeapon();
                            toggleCustomSpecial();
                        }, 50);
                ...
                if (data._backpack && Array.isArray(data._backpack)) { backpackItems = data._backpack;
                    renderBackpack(); }
                if (data._soulDevices && Array.isArray(data._soulDevices)) { soulDevices = data._soulDevices;
                    renderSoulDevices(); }
                if (data._stageId) {
                    setTimeout(function() {
                        ...
                        if (data._stageEvent) {
                            setTimeout(function() {
                                fields.stageEventSelect.value = data._stageEvent;
                                onStageEventSelect();
                            }, 50);
                        }
                    }, 100);
                ...
                if (data._weaponLocked !== undefined) {
                    weaponLocked = data._weaponLocked;
                    updateWeaponLock();
                }
                var map = {
                    'cn': '基本信息.姓名',
                    ...
                    'fc': '背景与阵营.所属阵营',
                    'fbg': '背景与阵营.阵营身份',
                    'orgIdentity': '背景与阵营.组织身份',
                    'reputation': '背景与阵营.阵营地位',
                    'bloodAwaken': '背景与阵营.血脉觉醒',
                    'msn': '武魂与魂力.武魂名称',
                    'mst': '武魂与魂力.武魂类型',
                    'isp': '武魂与魂力.先天魂力',
                    'csr': '武魂与魂力.当前魂力等级',
                ...
                if (data.第二职业) {
                    if (data.第二职业.职业类型) fields.secondProf.value = data.第二职业.职业类型;
                    if (data.第二职业.等级) {
                        var lv = data.第二职业.等级.replace('级', '');
                        var opts = Array.from(fields.secondLevel.options);
                        var match = opts.find(function(o) { return o.value === lv; });
                        if (match) fields.secondLevel.value = lv;
                    }
                }
                if (data.机甲) {
                    var mech = data.机甲.机甲等级 || '';
                    var found = false;
                    var opts = Array.from(fields.mechaLevel.options);
                    for (var i = 0; i < opts.length; i++) {
                        if (opts[i].value === mech) { fields.mechaLevel.value = mech;
                            found = true; break; }
                    }
                    if (!found && mech.startsWith('红级·')) {
                        fields.mechaLevel.value = '红级（自定义）';
                        fields.mechaCustom.value = mech.replace('红级·', '');
                        fields.mechaCustomWrap.style.display = 'block';
                    }
                }
                if (fields.og_select) {
                    var ogVal = getVal('og');
        ...
        function generatePersonaText() {
            ...
            if (d.基本信息.特质) lines.push('- 特质：' + d.基本信息.特质);
            if (d.基本信息.外貌) lines.push('- 外貌：' + d.基本信息.外貌);
            lines.push('');
            if (d.背景与阵营.出身地) lines.push('- 出身地：' + d.背景与阵营.出身地);
            if (d.背景与阵营.所属阵营 && d.背景与阵营.所属阵营 !== '无') lines.push('- 所属阵营：' + d.背景与阵营.所属阵营);
            if (d.背景与阵营.阵营身份 && d.背景与阵营.阵营身份 !== '无') lines.push('- 阵营身份：' + d.背景与阵营.阵营身份);
            if (d.背景与阵营.组织身份) lines.push('- 组织身份：' + d.背景与阵营.组织身份);
            if (d.背景与阵营.阵营地位 && d.背景与阵营.阵营地位 !== '无名小卒') lines.push('- 阵营地位：' + d.背景与阵营.阵营地位);
            if (d.背景与阵营.血脉觉醒 && d.背景与阵营.血脉觉醒 !== '未觉醒') lines.push('- 血脉觉醒：' + d.背景与阵营.血脉觉醒);
            lines.push('');
            lines.push('武魂与魂力：');
            var v = d.武魂与魂力;
            if (v.武魂类型 === '双生武魂') {
            ...
            if (v.当前魂力等级 && v.当前魂力等级 !== '未填写') {
                ...
                if (v.封号名称) lines.push('- 封号名称：' + v.封号名称);
            }
            lines.push('');
            if (d.精神力.精神力等级) {
                lines.push('精神力：');
            ...
            if (d.魂环 && d.魂环.length) {
                lines.push('');
                lines.push('魂环配置：');
                var colorMap = { '十年': '白', '百年': '黄', '千年': '紫', '万年': '黑', '十万年': '红', '凶兽（二十万年）': '橙金', '百万年': '彩' };
                d.魂环.forEach(function(r, idx) {
                    lines.push('- 第' + (idx + 1) + '魂环：' + r.年份 + '（' + (colorMap[r.年份] || '?') + '）');
                });
            }
            if (d.魂灵.主要魂灵名称 && d.魂灵.主要魂灵名称 !== '' && d.魂灵.主要魂灵名称 !== '未填写') {
                lines.push('');
                lines.push('魂灵：');
                var sc = { '十年': '白', '百年': '黄', '千年': '紫', '万年': '黑', '十万年': '红', '凶兽': '橙金', '百万年': '彩' };
            ...
            if (d.斗铠.斗铠名称 && d.斗铠.斗铠名称 !== '' && d.斗铠.斗铠名称 !== '未填写') {
                ...
                if (d.斗铠.斗铠部件完成) lines.push('- 斗铠部件完成度：' + d.斗铠.斗铠部件完成);
            }
            if (d.魂骨.已装备魂骨 && d.魂骨.已装备魂骨 !== '0/7') {
                lines.push('');
                lines.push('魂骨：');
                lines.push('- 已装备魂骨：' + d.魂骨.已装备魂骨);
                if (d.魂骨.魂骨列表 && d.魂骨.魂骨列表.length) {
                    lines.push('- 魂骨列表：' + d.魂骨.魂骨列表.join('、'));
                }
            }
            ...
            if (d.魂导器 && d.魂导器.length) {
                lines.push('- 魂导器：' + d.魂导器.map(function(s) { return s.name + '（' + s.quality + '）'; }).join('、'));
            }
            if (d.背包 && d.背包.length) {
                lines.push('- 背包：' + d.背包.join('、'));
            }
            if (d.降临阶段) {
                lines.push('');
                lines.push('╔═══════════════════════════╗');
            ...
            if (d.特殊能力 && d.特殊能力 !== '无' && d.特殊能力 !== '') {
                lines.push('');
                lines.push('特殊能力：' + d.特殊能力);
            }
            if (d.场景设定) {
                lines.push('');
                lines.push('场景：' + d.场景设定);
            }
            if (d.开场白) {
                lines.push('');
                lines.push('开场白：' + d.开场白);
            }
            return lines.join('\n');
        }
        ...
        function generateGreeting() {
            ...
            if (bg.阵营地位 && bg.阵营地位 !== '无名小卒') lines.push('【阵营地位】' + bg.阵营地位);
            if (bg.血脉觉醒 && bg.血脉觉醒 !== '未觉醒') lines.push('【血脉觉醒】' + bg.血脉觉醒);
            ...
            if (d.魂灵.主要魂灵名称 && d.魂灵.主要魂灵名称 !== '未填写' && d.魂灵.主要魂灵名称 !== '') {
                ...
                if (d.魂灵.其他魂灵.length) {
                    lines.push('【其他魂灵】' + d.魂灵.其他魂灵.join('、'));
                }
            }
            if (d.斗铠.斗铠名称 && d.斗铠.斗铠名称 !== '未填写' && d.斗铠.斗铠名称 !== '') {
                lines.push('【斗铠】' + d.斗铠.斗铠名称 + '（' + d.斗铠.斗铠等级 + '）');
            }
            if (d.魂骨.已装备魂骨 && d.魂骨.已装备魂骨 !== '0/7') {
                lines.push('【魂骨】已装备 ' + d.魂骨.已装备魂骨);
            }
            if (d.第二职业.职业类型 && d.第二职业.职业类型 !== '无') {
                lines.push('【第二职业】' + d.第二职业.职业类型 + '（' + d.第二职业.等级 + '）');
            }
            if (d.机甲.机甲等级 && d.机甲.机甲等级 !== '无') {
                lines.push('【机甲】' + d.机甲.机甲等级);
            }
            if (d.神器) lines.push('【神器】' + d.神器);
            if (d.特殊能力 && d.特殊能力 !== '无' && d.特殊能力 !== '') {
                lines.push('【特殊能力】' + d.特殊能力);
        ...
        function refreshExport() {
            exportPreview.value = generatePersonaText();
            var now = new Date();
        ...
        function fallbackCopy(text, silent) {
            ...
            try {
                if (document.execCommand('copy')) {
                    if (!silent) showMessage('✅ 已复制！');
                } else {
                    if (!silent) showMessage('⚠️ 手动复制');
                }
            } catch (_) {
                if (!silent) showMessage('⚠️ 手动复制');
            }
        ...
        function showMessage(text, isError) {
            ...
            msg._timer = setTimeout(function() { msg.style.display = 'none'; }, 2500);
        }
        ...
        function clearAll() {
            ...
            localStorage.setItem('soul_devices_v14', JSON.stringify(soulDevices));
            renderSoulDevices();
            armorPartNames.forEach(function(n) { armorParts[n] = false; });
            renderArmorParts();
        ...
        function bindEvents() {
            ...
            $('btnLockWeapon').addEventListener('click', function() {
                ...
                updateWeaponLock();
                saveDraft();
                showToast(weaponLocked ? '🔒 神器已锁定' : '🔓 神器自动模式已开启');
            });
            ...
            fields.fc.addEventListener('change', function() {
                updateIdentityByCamp();
                updateBackpackByCamp();
                updateSoulDevicesByCamp();
                if (this.value === '无') { fields.fbg.value = '无'; }
                saveDraft();
                updateProgress();
            });
            fields.fbg.addEventListener('change', function() {
                filterBackpackByIdentity();
                saveDraft();
            });
            ...
            fields.customSpecialName.addEventListener('input', function() { updateSpecialDesc();
                saveDraft(); });
            fields.customSpecialDesc.addEventListener('input', function() { updateSpecialDesc();
                saveDraft(); });
            ...
            fields.bal.addEventListener('change', function() { updateArmorGrade();
                saveDraft();
                updateProgress(); });
            fields.mechaLevel.addEventListener('change', function() { updateMechaCustom();
                saveDraft();
                updateProgress(); });
            fields.sp.addEventListener('change', function() { updateSpiritDesc();
                validateRings();
                saveDraft(); });
            fields.secondProf.addEventListener('change', function() { updateSecondProfDesc();
                saveDraft(); });
            fields.secondLevel.addEventListener('change', function() { updateSecondProfDesc();
                saveDraft(); });
            fields.ssy.addEventListener('change', function() { updateSpiritDot();
                saveDraft(); });
            ...
            $('btnAddBackpack').addEventListener('click', function() {
                var item = backpackInput.value.trim();
                if (item) { addBackpackItem(item);
                    backpackInput.value = ''; }
            });
            backpackInput.addEventListener('keydown', function(e) {
                if (e.key === 'Enter') { var item = this.value.trim(); if (item) { addBackpackItem(item);
                        this.value = ''; } }
            });

            $('btnAddSoulDevice').addEventListener('click', function() {
                var name = soulDeviceInput.value.trim();
                var quality = soulDeviceQuality.value;
                if (name) { addSoulDevice(name, quality);
                    soulDeviceInput.value = ''; }
            });
            soulDeviceInput.addEventListener('keydown', function(e) {
                if (e.key === 'Enter') { var name = this.value.trim(); var quality = soulDeviceQuality.value; if (name) { addSoulDevice(name, quality);
                        this.value = ''; } }
            });

            $('fateCopy').addEventListener('click', function() { copyToClipboard(fateBody.textContent);
                showToast('已复制天命！'); });
