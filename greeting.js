// ========== greeting.js - 斗罗大陆III 龙王传说 魂师觉醒 v6.0 FINAL ==========
(function(){
"use strict";
const styleText=`
...
#app{max-width:520px;width:100%;padding:14px 12px 90px 12px;background:linear-gradient(170deg,#1c130a,#23180d 30%,#1a1008 60%,#140c05);border:2px solid #7a5c2e;border-radius:14px;box-shadow:0 0 40px rgba(180,130,40,.18),0 8px 32px rgba(0,0,0,.55),inset 0 0 20px rgba(120,70,20,.08);position:relative;z-index:1;overflow:hidden;}
...
.special-tier{display:flex;gap:3px;flex-wrap:wrap;margin-bottom:2px;}
.special-tier .tier-tag{font-size:.5em;padding:1px 10px;border-radius:10px;cursor:pointer;transition:.2s;background:rgba(255,255,255,.02);color:#5a4a30;border:1px solid rgba(255,255,255,.04);}
.special-tier .tier-tag.t1{color:#7a7a7a;border-color:rgba(255,255,255,.05);}
.special-tier .tier-tag.t1.active{background:rgba(120,120,120,.15);border-color:#7a7a7a;color:#b5b5b5;}
.special-tier .tier-tag.t2{color:#c8963e;border-color:rgba(200,150,50,.1);background:rgba(200,150,50,.04);}
.special-tier .tier-tag.t2.active{background:rgba(200,150,50,.15);border-color:#c8963e;color:#f0d68a;}
.special-tier .tier-tag.t3{color:#b05a5a;border-color:rgba(200,80,80,.12);background:rgba(200,80,80,.04);}
.special-tier .tier-tag.t3.active{background:rgba(200,80,80,.15);border-color:#b05a5a;color:#dc6a6a;}
...
<div class="sec" data-step="6">
  ...
  <div class="fd"><label>特殊能力</label>
    ...
    <select id="st"></select>
    <div id="stDesc" style="font-size:.55em;color:#7a6a50;margin-top:2px;padding:2px 4px;background:rgba(0,0,0,.15);border-radius:3px;min-height:18px;font-style:italic;"></div>
  </div>
...
var toast=$('toast');
var STORAGE_KEY='soul_archive_v6';
...
// 魂环
var rings=[];
function initRings(){var saved=localStorage.getItem('soul_rings_v6');if(saved){try{rings=JSON.parse(saved);}catch(_){}}if(!rings.length)rings=[{year:'百年',name:'',effect:''}];renderRings();updateRingInfo();}
function renderRings(){ringContainer.innerHTML='';rings.forEach(function(ring,idx){var div=document.createElement('div');div.className='ring-item';var dotColor=getRingColor(ring.year);var dotClass=getRingClass(ring.year);div.innerHTML='<span class="dot '+dotClass+'" style="background:'+dotColor+';border-color:'+dotColor+';"></span><select data-ring-year="'+idx+'"><option value="十年"'+(ring.year==='十年'?'selected':'')+'>十年</option><option value="百年"'+(ring.year==='百年'?'selected':'')+'>百年</option><option value="千年"'+(ring.year==='千年'?'selected':'')+'>千年</option><option value="万年"'+(ring.year==='万年'?'selected':'')+'>万年</option><option value="十万年"'+(ring.year==='十万年'?'selected':'')+'>十万年</option><option value="凶兽（二十万年）"'+(ring.year==='凶兽（二十万年）'?'selected':'')+'>凶兽</option><option value="百万年"'+(ring.year==='百万年'?'selected':'')+'>百万年</option></select><input type="text" placeholder="魂技名" value="'+(ring.name||'')+'" data-ring-name="'+idx+'"><input type="text" placeholder="效果" value="'+(ring.effect||'')+'" data-ring-effect="'+idx+'"><button class="bm ring-del" data-ring-del="'+idx+'">✕</button>';ringContainer.appendChild(div);});ringContainer.querySelectorAll('[data-ring-year]').forEach(function(el){el.addEventListener('change',function(){var idx=Number(this.dataset.ringYear);rings[idx].year=this.value;localStorage.setItem('soul_rings_v6',JSON.stringify(rings));renderRings();updateRingInfo();updateProgress();saveDraft();showToast('魂环'+(idx+1)+' → '+this.value);});});ringContainer.querySelectorAll('[data-ring-name]').forEach(function(el){el.addEventListener('input',function(){var idx=Number(this.dataset.ringName);rings[idx].name=this.value;localStorage.setItem('soul_rings_v6',JSON.stringify(rings));saveDraft();updatePreview();});});ringContainer.querySelectorAll('[data-ring-effect]').forEach(function(el){el.addEventListener('input',function(){var idx=Number(this.dataset.ringEffect);rings[idx].effect=this.value;localStorage.setItem('soul_rings_v6',JSON.stringify(rings));saveDraft();});});ringContainer.querySelectorAll('[data-ring-del]').forEach(function(el){el.addEventListener('click',function(){var idx=Number(this.dataset.ringDel);if(rings.length<=1){showToast('至少保留一个魂环',true);return;}rings.splice(idx,1);localStorage.setItem('soul_rings_v6',JSON.stringify(rings));renderRings();updateRingInfo();updateProgress();saveDraft();showToast('已移除魂环');});});updateRingInfo();}
...
function syncRingsToLevel(){var lv=getVal('csr');var expected=getRingCount(lv);if(expected===0&&rings.length>0&&rings[0].name===''&&rings[0].effect===''){rings=[];}else if(expected>rings.length){for(var i=rings.length;i<expected;i++){rings.push({year:'百年',name:'',effect:''});}}else if(expected<rings.length&&expected>0){rings=rings.slice(0,expected);}localStorage.setItem('soul_rings_v6',JSON.stringify(rings));renderRings();updateRingInfo();updateProgress();saveDraft();showToast('🔄 已同步：'+rings.length+' 环');}
...
function updateTierUI(){
  var tags=document.querySelectorAll('.special-tier .tier-tag');
  tags.forEach(function(tag){
    tag.classList.remove('active');
    if(tag.dataset.tier===currentTier)tag.classList.add('active');
  });
}
...
function randomGenerate(){
  ...
  localStorage.setItem('soul_rings_v6',JSON.stringify(rings));renderRings();
...
function clearAll(){
  ...
  rings=[{year:'百年',name:'',effect:''}];localStorage.setItem('soul_rings_v6',JSON.stringify(rings));renderRings();
  ...
  currentTier='normal';updateTierUI();renderSpecialOptions();
}
...
function bindEvents(){
  ...
  stepPrev.onclick=function(){if(currentStep>1)goToStep(currentStep-1);};
  stepNext.onclick=function(){if(currentStep<totalSteps)goToStep(currentStep+1);};
  ...
  $('btnAddRing').addEventListener('click',function(){
    ...
    rings.push({year:'百年',name:'',effect:''});localStorage.setItem('soul_rings_v6',JSON.stringify(rings));
